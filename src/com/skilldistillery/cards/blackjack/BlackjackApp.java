...
public class BlackjackApp {
	...
	private void gameplayLoop(BlackJackTable theHouse, Scanner scan) {
		...
		boolean stay = false;
		while (theHouse.checkForBust() == false && stay == false) {
			if (theHouse.checkForDealerBlackjack() == true) {
				// house stays
				stay = hitOrStay(theHouse, scan, stay);
			} else if (theHouse.checkForPlayerBlackjack() == true) {
				stay = true;
				while (theHouse.checkForBust() == false && theHouse.dealer.dealerHand.getHandValue() <= 17) {
					theHouse.hitDealer();
				}
			} else {
				while (theHouse.checkForBust() == false && stay == false) {
...
//		while (stay == false || theHouse.checkForBust()== false) {
